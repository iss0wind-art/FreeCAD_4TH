"""
render_v12.py — v12 모델 matplotlib 렌더링 (headless)
"""
import os, sys, time
...
for name, elev, azim in VIEWS:
    ...
    ax.set_title(f'v12 북쪽주차장 확장 — {name.upper()}\n'
                 f'X {V[:,0].ptp():.0f}m × Y {V[:,1].ptp():.0f}m × Z {V[:,2].ptp():.1f}m  '
                 f'| 기둥1143+보778')
    out = f'output/v12_{name}.png'
