"""
build_v15_integrated.py — v5 정밀 모델링 통합 빌더 (공간 분할 런타임 최적화 적용)
========================================================================
...
6. 2D Spatial Grid Index를 통한 Boolean Cut 겹침 트리밍 런타임 최적화

실행:
  "C:/Program Files/FreeCAD 1.1/bin/freecadcmd.exe" tests/build_v15_integrated.py
"""
...
STEP_OUT = "output/v15_integrated.step"
BOQ_OUT = "output/v15_integrated_boq.json"
...
class CachedObj:
    """FreeCAD 객체의 3D Shape 및 BoundBox 속성을 캐싱하는 클래스."""
...
def main():
    t0 = time.time()
    print("=" * 60)
    print("v15: 정밀 모델링 통합 빌더 (공간 분할 최적화) 개시")
    print("=" * 60)
    ...
    # FreeCAD 도큐먼트 개설
    fc_doc = FreeCAD.newDocument("V15Integrated")
    shapes_with_names = []
    enriched_slabs = []
    slab_failed = 0
    ...
    # ── 3D Solid Overlap Trim (Boolean Cut) - 공간 분할 최적화 ──
    ...
    # 2. 보(BEAM) 트리밍 (보 ↔ 기둥) 및 보 그리드 인덱싱
    n_beam_trimmed = 0
    beam_grid: dict[tuple[int, int], list[CachedObj]] = {}
    cached_beams: list[CachedObj] = []
    ...
    for b in beam_objs:
        ...
        try:
            ...
            # 트리밍 후 최종 BoundBox 기반으로 보 그리드 인덱스 등록
            if trimmed:
                b_cached.update_bbox()
            cached_beams.append(b_cached)
            for key in b_cached.grid_keys:
                beam_grid.setdefault(key, []).append(b_cached)
        except Exception:
            pass

    # 3. 벽체(WALL) 트리밍 (벽체 ↔ 기둥, 보) 및 벽체 그리드 인덱싱
    n_wall_trimmed = 0
    wall_grid: dict[tuple[int, int], list[CachedObj]] = {}
    cached_walls: list[CachedObj] = []

    for w in wall_objs:
        if not hasattr(w, "Shape") or not w.Shape:
            continue
        try:
            w_cached = CachedObj(w)
            
            # 3-1. 기둥(COL) 후보군과 교차 트리밍
            col_candidates = set()
            for key in w_cached.grid_keys:
                if key in col_grid:
                    for c_cached in col_grid[key]:
                        col_candidates.add(c_cached)
            
            col_trimmed = False
            for c_cached in col_candidates:
                if intersect_bbox(w_cached.bbox_tuple, c_cached.bbox_tuple):
                    try:
                        cut_shape = w.Shape.cut(c_cached.obj.Shape)
                        if cut_shape and len(cut_shape.Solids) > 0:
                            w.Shape = cut_shape
                            col_trimmed = True
                            n_wall_trimmed += 1
                        ...
                        pass
            if col_trimmed:
                w_cached.update_bbox()

            # 3-2. 보(BEAM) 후보군과 교차 트리밍
            beam_candidates = set()
            for key in w_cached.grid_keys:
                if key in beam_grid:
                    for b_cached in beam_grid[key]:
                        beam_candidates.add(b_cached)
            
            beam_trimmed = False
            for b_cached in beam_candidates:
                if intersect_bbox(w_cached.bbox_tuple, b_cached.bbox_tuple):
                    try:
                        cut_shape = w.Shape.cut(b_cached.obj.Shape)
                        if cut_shape and len(cut_shape.Solids) > 0:
                            w.Shape = cut_shape
                            beam_trimmed = True
                            n_wall_trimmed += 1
            ...
            cached_walls.append(w_cached)
            for key in w_cached.grid_keys:
                wall_grid.setdefault(key, []).append(w_cached)
        except Exception:
            pass

    # 4. 슬라브(SLAB) 트리밍 (슬라브 ↔ 기둥, 보, 벽체)
    n_slab_trimmed = 0
    for s in slab_objs:
        if not hasattr(s, "Shape") or not s.Shape:
            continue
        try:
            s_cached = CachedObj(s)
            
            # 4-1. 기둥(COL) 커팅
            col_candidates = set()
            for key in s_cached.grid_keys:
                if key in col_grid:
                    for c_cached in col_grid[key]:
                        col_candidates.add(c_cached)
            ...
            col_trimmed = False
            for c_cached in col_candidates:
                if intersect_bbox(s_cached.bbox_tuple, c_cached.bbox_tuple):
                    try:
                        cut_shape = s.Shape.cut(c_cached.obj.Shape)
                        if cut_shape and len(cut_shape.Solids) > 0:
                            s.Shape = cut_shape
                            col_trimmed = True
                            n_slab_trimmed += 1
            ...
            beam_trimmed = False
            for b_cached in beam_candidates:
                if intersect_bbox(s_cached.bbox_tuple, b_cached.bbox_tuple):
                    try:
                        cut_shape = s.Shape.cut(b_cached.obj.Shape)
                        if cut_shape and len(cut_shape.Solids) > 0:
                            s.Shape = cut_shape
                            beam_trimmed = True
                            n_slab_trimmed += 1
            ...
            for key in s_cached.grid_keys:
                if key in wall_grid:
                    for w_cached in wall_grid[key]:
                        wall_candidates.add(w_cached)
            
            for w_cached in wall_candidates:
                if intersect_bbox(s_cached.bbox_tuple, w_cached.bbox_tuple):
                    try:
                        cut_shape = s.Shape.cut(w_cached.obj.Shape)
                        if cut_shape and len(cut_shape.Solids) > 0:
                            s.Shape = cut_shape
                            n_slab_trimmed += 1
                    except Exception:
