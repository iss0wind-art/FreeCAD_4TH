...
class FrameData:
    ...
    def summary(self) -> str:
        lines = [
            ...
            f'  기둥 {len(self.columns)}개 | 보 {len(self.beams)}개 | '
    ...
    def to_json(self, indent: int = 2) -> str:
        ...
        def slab_dict(s: SlabOutline) -> dict:
            return {'area_m2': round(s.area_m2, 1), 'pts_count': len(s.pts),
...
def parse_structural_frame(
    ...
    structural = extractor.extract(doc, clip=clip)
    ...
    if extract_grid:
        ...
        frame.grid = _extract_grid(doc, clip=clip)
